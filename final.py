...
@author: Patrick
Calculates the attenutation lengths of different materials
"""
...
def random_walk(finished, abs_check):
     """
     performs a random walk until the neutron is either absorbed, transmitted or 
     relflected and returns the result
     """
     x = [0]
     y = [0]
     z = [0]
...
for i in range(1):
    ...
    #changes the data used depending on the loop number
    if i == 0:
    ...
    for k in range(30):
        ...
        #does a random walk for each neutron            
        ...
        #calculated the error and percent of neutrons for each option
    ...
    #Polyfit obtains final errors and chi square values
    fit, cov = np.polyfit(tran_len, trans_log, deg=1, w=1/trans_log_err, cov=True)
    chi_square_tot= np.polyfit(tran_len, trans_log, deg=1, full=True, w=1/trans_log_err)[1][0]

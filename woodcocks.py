...
lmda2, abs_check2, material2, Sig_total2 = lead()
...
for j in range(nneutrons):
     ...
     while not absorbed and not transmitted and not reflected:
        if maj_xsec == Sig_total:
            ...
            r = -lmda2 * np.log(np.random.uniform(0, 1, 1))
        ...
        if np.random.uniform(0, 1, size = 1) > fict_prob:
            fict_step = False
        else:
            fict_step = True
            
        if fict_step or (origin == position).all():
            step_size = r * random_direction(1)
...
trans_percent = (trans_tot / nneutrons) * 100
trans_err = np.sqrt(trans_tot * trans_percent*(1 - trans_percent))

print(trans_percent,"pm",trans_err)

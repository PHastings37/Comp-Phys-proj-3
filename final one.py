# -*- coding: utf-8 -*-
"""
Created on Mon May 11 12:53:12 2020
...
def random_walk(finished, abs_check):
    """
    performs a random walk until the parctilce is either absorbed transmitted 
    or reflected then returns the result
    """
     x = [0]
     y = [0]
     z = [0]
...
#initialises the length and number of neutrons being used
length = 0.01
nneutrons = 1000

for i in range(1):
    """
    Loops for 3 times, 1 for each material
    """
    #Initialises all the variables for use after each loop
    lmda = 0
    abs_check = 0
    ...
    #depending on the loop number, it changes to a different data set
    if i == 0:
    ...
    for k in range(10):
        #iterates over a range of lengths
        length += 0.01
